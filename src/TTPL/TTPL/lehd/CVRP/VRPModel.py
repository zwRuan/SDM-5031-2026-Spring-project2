    ...
    def forward(
        ...
        **kwargs
        ...
        if self.mode == "test":
            ...
            )  # node indices greater than the customer count refer to the depot
...
class CVRP_Decoder(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        selected_node_list_ = selected_node_list.clone().detach() - 1  # convert to zero-based index
        data_ = problems[:, 1:, :].clone().detach()  # data includes the depot
        batch_size_V = data_.shape[0]  # B
        problem_size = data_.shape[1]

        knearest_num = self.model_params["k_nearest_nodes"]
        new_data_len = problem_size - selected_node_list_.shape[1]

        left_node_coor = self._get_new_data(
            problems[:, 1:, :2], selected_node_list_, problem_size, batch_size_V
        )
        left_node = self._get_new_data(
            problems[:, 1:, :], selected_node_list_, problem_size, batch_size_V
        )
        if selected_node_list_.shape[1] == 0:
            last_node = problems[:, [0], :]
        else:
            last_node = self._get_encoding(
                problems[:, 1:, :], selected_node_list_[:, [-1]]
            )

        last_node_coor = last_node[:, :, :2]

        if self.model_params["knearest"]:
            if new_data_len > knearest_num:
                distance = torch.sum((left_node_coor - last_node_coor) ** 2, dim=2).sqrt()  # [B,new_data_len]

                sort_value, sort_index = torch.topk(distance, k=knearest_num, dim=1, largest=False)
                k_nearest_point = self._get_encoding(left_node, sort_index)

                left_node = k_nearest_point.clone().detach()

            if self.model_params['coor_projection'] and projection is not None:
                problems[:, [0], :2], left_node[:, :, :2], last_node[:, :, :2] = \
                    projection(problems[:, [0], :2], left_node[:, :, :2], last_node[:, :, :2])

        if MVDF:
            encoded_nodes = encoder(
                torch.cat((MVDF_POMO(problems[:, [0], :]), MVDF_POMO(left_node), MVDF_POMO(last_node)), dim=1), capacity
            )
        else:
            encoded_nodes = encoder(
                torch.cat((problems[:, [0], :], left_node, last_node), dim=1), capacity
            )

        embedded_first_node = encoded_nodes[:, [0], :]
        embedded_last_node = encoded_nodes[:, [-1], :]
        left_encoded_node = encoded_nodes[:, 1:-1, :]

        remaining_capacity = remaining_capacity.reshape(batch_size_V, 1, 1) / capacity
        if MVDF:
            remaining_capacity = remaining_capacity.repeat(8, 1, 1)
        ...
        if self.model_params["knearest"]:
            if new_data_len > knearest_num:
                ...
                # shape: [B*(V-1), problem_size]; fill probabilities for the remaining nodes
                # construct high-dimensional indices for scatter assignment
                index_1_ = torch.arange(batch_size_V, dtype=torch.long)[:, None].expand(
                    batch_size_V, sort_index.shape[1] * 2
                )  # shape: [B*(V-1), n]

                index_2_ = torch.cat(
                    (
                        (sort_index).type(torch.long),
                        (new_data_len) + (sort_index).type(torch.long),
                    ),
                    dim=-1,
                )
...
def MVDF_POMO(problems):
    ...
    y = problems[:, :, [1]]
    z = problems[:, :, [2]]
    dat1 = torch.cat((x, y, z), dim=2)
    dat2 = torch.cat((1 - x, y, z), dim=2)
    dat3 = torch.cat((x, 1 - y, z), dim=2)
    dat4 = torch.cat((1 - x, 1 - y, z), dim=2)
    dat5 = torch.cat((y, x, z), dim=2)
    dat6 = torch.cat((1 - y, x, z), dim=2)
    dat7 = torch.cat((y, 1 - x, z), dim=2)
    dat8 = torch.cat((1 - y, 1 - x, z), dim=2)
