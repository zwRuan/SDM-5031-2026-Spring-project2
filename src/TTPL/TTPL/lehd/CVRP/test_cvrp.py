...
from lehd.CVRP.VRPTester import VRPTester as Tester  # noqa: E402
from lehd.utils.utils import create_logger  # noqa: E402
from lehd.CVRP import projection  # noqa: E402
...
# Format: {problem_size: [test_file, test_episodes, batch_size, begin_index]}
test_paras = {
    1000: ["test/vrp1000_train_hgs_resolve_GLOP_n100.txt", 100, 100, 0],
    5000: ["test/vrp5000_train_hgs_resolve_GLOP_n100.txt", 100, 100, 0],
    10000: ["test/vrp10000_test_n16_solution_flag.txt", 16, 16, 0],
    50000: ["test/New_Generated_vrp50000_test_uniform_hgs_n16_C300.txt", 16, 16, 0],
    100000: ["test/New_Generated_vrp100000_test_uniform_hgs_n16_C300.txt", 16, 8, 0],
    0: ["test/CVRPlib_scale_larger_than1000_Li_X_XXL_n14.txt", 14, 1, 0],
}

# Environment parameters for the CVRP tester
env_params = {
    "mode": "test",
    "sub_path": False,
}

# Model parameters for the CVRP model
model_params = {
    "mode": "test",
    "embedding_dim": 128,
...
def main_test(args, **kwargs):
    """
    Main function to run the CVRP test.
    """
    if DEBUG_MODE:
        _set_debug_mode()
    ...
    tester_params["model_load"] = {
        "path": os.path.join(project_root, "lehd", "CVRP", args.model_load_path),
        "epoch": args.model_load_epoch,
    }

    # Configure logger description based on arguments
    logger_params["log_file"]["desc"] = (
        f"test_counter_{args.counter_current}_vrplib{args.test_in_vrplib}_vrp{args.problem_size}_"
    ...
    env_params["vrplib_path"] = os.path.join(script_dir, "data", data_filename)
...
def add_common_args(parser):
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Whether to test on CVRPlib instances",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--model_load_path",
        type=str,
        default="result/CVRP100_model",
        help="Path to the model to load",
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test script for CVRP")
