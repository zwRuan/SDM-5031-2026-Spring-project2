...
Non-uniform angular sampling with concentration near cardinal directions.
"""
import torch
import numpy as np
...
def generate_augmentations(coords, aug_factor=256):
    ...
    for i in range(n):
        quadrant = i % 4
        idx_in_quad = i // 4
        total_per_quad = n // 4 + 1
        t = (idx_in_quad + 0.5) / total_per_quad
        t_pow = t ** 0.7
        angle = (quadrant + t_pow) * np.pi / 2
