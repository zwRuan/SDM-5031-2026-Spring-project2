...
Halton rotations with mild anisotropic x-axis scaling for aspect-ratio diversity.
"""
import torch
import numpy as np
...
def generate_augmentations(coords, aug_factor=256):
    ...
    for i in range(aug_factor - 8):
        ...
        scale_x = 0.9 + 0.2 * halton(i + 1, 3)
        c, s = np.cos(angle), np.sin(angle)
        centered = coords - 0.5
        cen_scaled = centered.clone()
        cen_scaled[:, :, 0] = centered[:, :, 0] * scale_x
        xr = cen_scaled[:, :, 0] * c - cen_scaled[:, :, 1] * s
        yr = cen_scaled[:, :, 0] * s + cen_scaled[:, :, 1] * c
